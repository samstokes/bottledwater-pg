...
describe 'topics', functional: true do
  ...
  after(:example) do
    TEST_CLUSTER.send(:services, /bottledwater-json/).each {|container| TEST_CLUSTER.send(:dump_container_logs, container) } # TODO

    # since we're testing crashes...
    unless TEST_CLUSTER.healthy?
      TEST_CLUSTER.restart#(dump_logs: false) # TODO
    end
  end
