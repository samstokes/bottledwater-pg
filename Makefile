...
docker: docker-client docker-postgres docker-postgres95 docker-postgres94
...
tmp/%-95.tar.gz: tmp docker-build-95
	docker run --rm bwbuild-95:$(DOCKER_TAG) cat /$*-95.tar.gz > $@
...
docker-build-95:
	docker build -f build/Dockerfile.build95 -t bwbuild-95:$(DOCKER_TAG) .
...
docker-postgres95: tmp/Dockerfile.postgres95 tmp/bottledwater-ext-95.tar.gz tmp/avro.tar.gz tmp/replication-config.sh
	docker build -f $< -t local-postgres95-bw:$(DOCKER_TAG) tmp
